...
payments as (
    ...
    WHERE payments.state = 'posted'
        AND payments.transaction_date >= CURRENT_DATE - INTERVAL '6 MONTH' --and customer.unique_account_id = 'BXCK00010444'
...
active as (
    ...
    from kenya.agg_dcs_today as today
        left join kenya.customer as customer on customer.customer_id = today.customer_id
),
collection_rate as (
    ...
    from active as active
        ...
        left join kenya.rp_portfolio_customer_lookup as look on look.customer_id = active.customer_id --limit 5
),
-----------------old query------------
last_30_day_CR AS (
    SELECT payments.payg_account_id,
...
dataset as (
    select collection_rate.customer_id,
        ...
        CASE
            WHEN collection_rate.installation_date + 30 > collection_rate.activity_date THEN 'New Customer'
            ELSE 'Older Customer'
