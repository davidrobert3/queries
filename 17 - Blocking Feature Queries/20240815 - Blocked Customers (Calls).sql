WITH customer_phones AS (
	-- Retrieve unique customer IDs with their phone numbers and alternative phones
	...
	WHERE ccp.unique_customer_id IN (
			...
			WHERE month_ = 8 -- Filter for calls made in August
		)
	GROUP BY 1,
		2,
		3
...
customer_par AS (
	-- Join customer phone details with call list to include par_bucket
	SELECT cp.*,
		-- All fields from customer_phones
		ccl.par_bucket -- Par bucket from the call list
	FROM customer_phones cp
		LEFT JOIN cscc_call_list ccl ON cp.unique_customer_id = ccl.unique_account_id
	WHERE month_ = 8 -- Filter for August
),
-----------------------------------------------------
...
call_log_data AS (
	...
	WHERE caller_id IN (
		...
		AND call_time::DATE >= '2024-08-14' -- Start date
	GROUP BY 1,
		2,
		3,
		4,
...
consolidated_customer_bxck AS (
	-- Consolidate customer IDs and par buckets from all phone types
	SELECT call_time::DATE,
		-- Date of the call
		caller_id,
		-- ID of the caller
...
SELECT DISTINCT caller_id,
	-- ID of the caller
	customer_bxck -- Consolidated customer ID
