...
with downpayments as (
	...
	where
		downpayment_date :: date >= '20240517' - 180
		and downpayment_date :: date <= '20240517'
	group by
		1,
		2
),
...
payments as (
	...
	where
		...
		and payments.payment_utc_timestamp :: DATE >= '20240517' - 180
		and payments.payment_utc_timestamp :: DATE <= '20240517'
	group by
		payments.sales_order_id,
		downpayments.total_downpayment
),
customers as (
	select
		...
		c.unique_account_id,
		case
			when c.customer_active_end_date :: DATE >= '20240517' then null
			else c.customer_active_end_date :: DATE
		end as customer_active_end_date,
		c.customer_active_start_date,
		case
			when c.customer_active_end_date :: DATE >= '20240517' then null
			else c.customer_final_status
		end as customer_final_status
	from
		kenya.customer c
),
...
active as (
	select
		...
		least(
			(
				(
					'20240517' - (customer.customer_active_start_date :: DATE + 7)
				) :: BIGINT
			),
			180
		) as days_expected
	from
		...
		left join customers as customer on customer.account_id = dcs.account_id
	...
	where
		...
		and dcs.date_timestamp :: DATE = '20240517'
...
last_30_downpayments as (
	...
	from
		kenya.rp_retail_sales
	where
		downpayment_date :: date >= '20240517' - 30
		and downpayment_date :: date <= '20240517'
	group by
		1,
		2
...
last_30_day_CR as (
	...
	where
		payments.processing_status = 'posted'
		and payments.is_void = false
		and payments.payment_utc_timestamp :: DATE >= '20240517' - 30
		and payments.payment_utc_timestamp :: DATE <= '20240517'
	group by
		payments.sales_order_id,
...
legacy_repos as (
	select
		'20240517' :: DATE as activity_date,
	...
	where
		rpcl.customer_active_end_date >= '20240516'
		and rpcl.customer_active_end_date <= '20240517' - 1
...
full_data as (
	...
	where
		esf_defaulters = 0 -- Excluding ESF defaulters
		--	limit 5
	union
	all
	select
