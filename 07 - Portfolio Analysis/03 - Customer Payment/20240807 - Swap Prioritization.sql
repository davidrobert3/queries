...
SELECT
    ...
    datediff(day,
    s.installation_utc_timestamp::date,
    smc.date_done::DATE) as days_active,
    s.downpayment,
    ...
    case
        ...
        s.installation_utc_timestamp::date,
        smc.date_done::DATE))- s.downpayment_credit_amount)) + s.downpayment > s.total_value
                then s.total_value
        else (s.credit_price * ((datediff(days,
        s.installation_utc_timestamp::date,
        smc.date_done::DATE))- s.downpayment_credit_amount)) + s.downpayment
...
GROUP BY 1,2,3,4,5,6,7,8,shop,10,11
--LIMIT 5