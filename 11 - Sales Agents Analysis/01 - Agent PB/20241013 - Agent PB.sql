...
payments AS (
    SELECT payments.sales_order_id AS payg_account_id,
    ...
    WHERE payments.is_void IS FALSE
    ...
    GROUP BY payments.sales_order_id,
        downpayments.total_downpayment
    ORDER BY payments.sales_order_id
),
---------------------------------------------
-- Add agent reassignment/remapping CTE. Add new 
sales_agents as (
    select sa.sales_agent_id,
        sa.sales_agent_name,
...
rp_sales as (
    select sales.unique_account_id,
        case
            ...
            when upper(substring(sales.sales_person, 0, 7)) in ('KE4919', 'KE0544','KE0662', 'KE0585') then 'KE7925'
            ...
            when upper(substring(sales.sales_person, 0, 7)) in ('KE5388', 'KE0796', 'KE2218', 'KE3552', 'KE6709', 'KE0736', 'KE3343', 'KE7277', 'KE7296', 'KE2215','KE1111','KE1285') then 'KE7742'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE4820', 'KE4822', 'KE6342') then 'KE6898'
            ...
            when upper(substring(sales.sales_person, 0, 7)) in ('KE4820', 'KE4822', 'KE6342') then 'KE6878'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE7962') then 'KE0598'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE7976') then 'KE0121'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE8007') then 'KE3305'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE3632') then 'KE8013'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE7962') then 'KE0598'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE7976') then 'KE0121'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE8007') then 'KE3305'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE3632') then 'KE8013'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE4919','KE0544','KE0662','KE0024','KE0585') then 'KE7965'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE0872','KE0743','KE0749', 'KE0744') then 'KE8034'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE1145','KE6840') then 'KE8023'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE0483','KE4499','KE2280','KE7847','KE0774','KE3663','KE0190','KE1469','KE4029','KE4221') then 'KE4936'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE0201' ,'KE0402', 'KE0088','KE1468', 'KE3664','KE3824' ,'KE4907','KE5119','KE7813','KE7814','KE2279', 'KE0826') then 'KE7693'
            when upper(substring(sales.sales_person, 0, 7)) in ('KE0098') then 'KE7995'
...
left join kenya.customer as customer on
...
from
	active as active
left join payments as repayments on
...
select
	...
	SUM(amount) - (
            coalesce(last_30_downpayments.total_downpayment,
	0)
        ) as payments
from
	kenya.payment as payments
left join last_30_downpayments as last_30_downpayments on
	last_30_downpayments.unique_account_id = payments.sales_order_id
where
...
left join last_7_day_CR as last_7_day_CR on
